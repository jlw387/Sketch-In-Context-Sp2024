...
# Plot all runs
for i in range(len(training_elbos)):
    plt.plot(training_elbos[i], color=(0.3,0.5,1.0), label="Training" if i == 0 else None)
    plt.plot(test_elbos[i], color=(1.0,0.5,0.0), label="Test" if i == 0 else None)
    

plt.title("Network Training/Test Performance")
plt.legend()
plt.show()
...
#     plt.plot(te, color=color_list[counter % 4], label="LD-" + str(latent_dims[counter % 4]) if counter < 4 else None)
#     counter += 1
