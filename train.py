...
def train(network_depth, latent_dims, filter_size, final_grid_size, sketch_points_dir_train, sketch_points_dir_test, batch_size_param, learning_rate, total_epochs, run_description, use_augmentation = True):
    # DATA COLLECTION PARAMETERS
    train_split = 0.9 # PARAM
    ...
    # Load training and test datasets
    train_dataset = dataset.SketchPointDataset(sketch_points_dir_train, use_augmentation, device=DEVICE)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_param, shuffle=True)

    test_dataset = dataset.SketchPointDataset(sketch_points_dir_test, use_augmentation, 512, device=DEVICE)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size_param, shuffle=False)
    # total_dataset = dataset.SketchPointDataset(data_dir, train_split, device=DEVICE)
    # dataset_loader = DataLoader(total_dataset, batch_size=batch_size_param, shuffle=False)
    ...
    coord = torch.Tensor([[0, 0, 0]])  # place holder for coordinate data
    ...
    for idx, image_batch in enumerate(test_dataloader):
        ...
        # Forward propagate image batch
        reprojections, z, mu, logVar = network(image_batch, coord)
...
if __name__ == "__main__":
    ...
    train(network_depth, latent_dims, filter_size, final_grid_size, sketch_points_dir_train, sketch_points_dir_test, batch_size_param, learning_rate, total_epochs, "data check")